...
const Alphynweb_Event_Start_Date_Meta_Fields = ({ postType, postMeta, setPostMeta }) => {
    ...
    const togglePopoverButtonText = isPopoverVisible ? "Done" : "Set start time and date of event";

    const storedDate = postMeta._event_start_date ? postMeta._event_start_date : date;

    const onUpdateDateTime = (newDateTime) => {
        setDate(newDateTime);
        setPostMeta({_event_start_date: newDateTime});
    };

    return(
            <PluginDocumentSettingPanel title={ __('Start date and time', 'alphynweb-calendar-events') } icon="edit" initialOpen="false">
                <PanelRow>
...
])(Alphynweb_Event_Start_Date_Meta_Fields);
